// Copyright 2026 Alex Stockwell and pkgmirror contributors.
// Portions Copyright 2023 The Gitea Authors.
// SPDX-License-Identifier: MIT
//
// Ported from forgejo/modules/packages/rpm/metadata_test.go (MIT).
// The base64 RPM blob is preserved verbatim so we exercise the exact
// same bytes the upstream parser was validated against. A drifting
// fixture would silently change what we're really testing.

package rpm

import (
	"bytes"
	"compress/gzip"
	"encoding/base64"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

// base64RpmPackage is a real .rpm built by KN4CK3R (Forgejo upstream)
// for parser-conformance testing. gzipped to keep the test file
// readable; gunzipped at test time. Don't change — every byte is
// load-bearing for the field-by-field assertions below.
const base64RpmPackage = ``

func TestParsePackage(t *testing.T) {
	rpmPackageContent, err := base64.StdEncoding.DecodeString(base64RpmPackage)
	require.NoError(t, err)

	zr, err := gzip.NewReader(bytes.NewReader(rpmPackageContent))
	require.NoError(t, err)

	p, err := ParsePackage(zr)
	require.NoError(t, err)
	require.NotNil(t, p)

	assert.Equal(t, "gitea-test", p.Name)
	assert.Equal(t, "1.0.2-1", p.Version)
	require.NotNil(t, p.VersionMetadata)
	require.NotNil(t, p.FileMetadata)

	assert.Equal(t, "MIT", p.VersionMetadata.License)
	assert.Equal(t, "https://gitea.io", p.VersionMetadata.ProjectURL)
	assert.Equal(t, "RPM package summary", p.VersionMetadata.Summary)
	assert.Equal(t, "RPM package description", p.VersionMetadata.Description)

	assert.Equal(t, "x86_64", p.FileMetadata.Architecture)
	assert.Equal(t, "0", p.FileMetadata.Epoch)
	assert.Equal(t, "1.0.2", p.FileMetadata.Version)
	assert.Equal(t, "1", p.FileMetadata.Release)
	assert.Empty(t, p.FileMetadata.Vendor)
	assert.Equal(t, "KN4CK3R", p.FileMetadata.Packager)
	assert.Equal(t, "gitea-test-1.0.2-1.src.rpm", p.FileMetadata.SourceRpm)
	assert.Equal(t, "e44b1687d04b", p.FileMetadata.BuildHost)
	assert.EqualValues(t, 1678225964, p.FileMetadata.BuildTime)
	assert.EqualValues(t, 1678225964, p.FileMetadata.FileTime)
	assert.EqualValues(t, 13, p.FileMetadata.InstalledSize)
	assert.EqualValues(t, 272, p.FileMetadata.ArchiveSize)
	assert.Empty(t, p.FileMetadata.Conflicts)
	assert.Empty(t, p.FileMetadata.Obsoletes)

	assert.ElementsMatch(t, []*Entry{
		{Name: "gitea-test", Flags: "EQ", Version: "1.0.2", Epoch: "0", Release: "1"},
		{Name: "gitea-test(x86-64)", Flags: "EQ", Version: "1.0.2", Epoch: "0", Release: "1"},
	}, p.FileMetadata.Provides)

	assert.ElementsMatch(t, []*Entry{
		{Name: "/bin/sh"},
		{Name: "/bin/sh"},
		{Name: "/bin/sh"},
		{Name: "rpmlib(CompressedFileNames)", Flags: "LE", Version: "3.0.4", Epoch: "0", Release: "1"},
		{Name: "rpmlib(FileDigests)", Flags: "LE", Version: "4.6.0", Epoch: "0", Release: "1"},
		{Name: "rpmlib(PayloadFilesHavePrefix)", Flags: "LE", Version: "4.0", Epoch: "0", Release: "1"},
		{Name: "rpmlib(PayloadIsXz)", Flags: "LE", Version: "5.2", Epoch: "0", Release: "1"},
	}, p.FileMetadata.Requires)

	assert.ElementsMatch(t, []*File{
		{Path: "/usr/local/bin/hello", IsExecutable: true},
	}, p.FileMetadata.Files)

	assert.ElementsMatch(t, []*Changelog{
		{Author: "KN4CK3R <dummy@gitea.io>", Date: 1678276800, Text: "- Changelog message."},
	}, p.FileMetadata.Changelogs)
}

func TestParsePackage_InvalidProjectURLDropped(t *testing.T) {
	// Same hygiene as the other parsers: a non-http URL on the
	// project metadata gets cleared rather than failing the
	// upload. We can't easily construct a real .rpm with a bad
	// URL here — exercise the helper directly.
	if isValidProjectURL("not-a-url") {
		t.Fatal("expected non-URL to be rejected")
	}
	if isValidProjectURL("ftp://example.com") {
		t.Fatal("expected ftp scheme to be rejected")
	}
	if !isValidProjectURL("https://example.com/foo") {
		t.Fatal("expected https URL to be accepted")
	}
	if !isValidProjectURL("http://example.com") {
		t.Fatal("expected http URL to be accepted")
	}
}
