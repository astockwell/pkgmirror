...
//
// This file is a transliteration of forgejo/modules/packages/rubygems/marshal.go
// from the Forgejo project (https://codeberg.org/forgejo/forgejo), which is
// itself MIT licensed. The MarshalEncoder below preserves the upstream
// algorithm verbatim; differences are limited to the package name,
// import paths (no util.NewInvalidArgumentErrorf — we use plain errors.New
// since our sentinel error model is simpler), and zero functional changes.
//
// This is intentionally a minimal Ruby Marshal v4.8 encoder — just enough
// to satisfy the `gem` and Bundler RubyGems registry clients. Decoder is
// not implemented because no endpoint we serve needs to read Marshal data.
...
// ErrUnsupportedType indicates an unsupported type was passed to the encoder.
var ErrUnsupportedType = errors.New("rubygems marshal: type is unsupported")

// ErrInvalidIntRange indicates an integer outside the encodable range
// (Ruby Marshal Fixnums are -2^30..2^30-1).
var ErrInvalidIntRange = errors.New("rubygems marshal: integer not in valid range")

// RubyUserMarshal is a Ruby object whose class implements marshal_load.
type RubyUserMarshal struct {
	Name  string
	Value any
}

// RubyUserDef is a Ruby object whose class implements _load.
type RubyUserDef struct {
	Name  string
	Value any
}

// RubyObject is a default Ruby object with named instance variables.
type RubyObject struct {
	Name   string
	Member map[string]any
}

// MarshalEncoder mimics Ruby's Marshal class.
// Only supports the types used by the RubyGems package registry protocol.
type MarshalEncoder struct {
	w       *bufio.Writer
	symbols map[string]int
}

// NewMarshalEncoder creates a new MarshalEncoder writing to w.
...
// Encode writes the Marshal version prefix and encodes v.
...
func (e *MarshalEncoder) marshalIntInternal(i int64) error {
	...
	case 122 < i && i <= 0xff:
		length = 1
	case 0xff < i && i <= 0xffff:
		length = 2
	case 0xffff < i && i <= 0xffffff:
		length = 3
	case 0xffffff < i && i <= 0x3fffffff:
		length = 4
	case -0x100 <= i && i < -123:
		length = -1
	case -0x10000 <= i && i < -0x100:
		length = -2
	case -0x1000000 <= i && i < -0x100000:
		length = -3
	case -0x40000000 <= i && i < -0x1000000:
		length = -4
	default:
